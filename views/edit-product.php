...
$stmt = $con->prepare('SELECT * FROM products WHERE product_id=:id');
...
if (isset($_POST['submit'])) {
    ...
    if (!empty($_FILES['images']['name'])) {
        ...
        if (move_uploaded_file($uploadedFile, $targetFile)) {
            ...
            $sql = "UPDATE products SET product_name=:a, category=:b, price=:c, description=:d, images=:e, width=:f, height=:g, depth=:h, weight=:i, quantity=:j WHERE product_id=:id";
        ...
        $sql = "UPDATE products SET product_name=:a, category=:b, price=:c, description=:d, width=:f, height=:g, depth=:h, weight=:i, quantity=:j WHERE product_id=:id";
        $stmt = $con->prepare($sql);
        $stmt->bindParam(':a', $product_name);
        $stmt->bindParam(':b', $category);
        $stmt->bindParam(':c', $price);
...
include_once '../templates/edit-product-template.php';
?>
