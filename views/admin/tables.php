...
if(isset($_POST['create-upcoming'])){
    $product_name = $_POST['product_name'];
    $category = $_POST['category'];
    $price = $_POST['price'];
    $description = $_POST['description'];
    ...
    // File upload handling
$uploadDir = '../../public/img/upcoming/'; // Change this to your desired upload directory
...
if (move_uploaded_file($uploadedFile, $targetFile)) {
    // File was uploaded successfully, continue with database insert
    $sql = "INSERT INTO upcoming_products(product_name, category, price, description, images_upcoming, width, height, depth, weight, quantity) 
            VALUES (:a, :b,:c,:d,:e,:f,:g,:h,:i,:j)";
    $stmt = $con->prepare($sql);
    $stmt->bindParam(':a', $product_name);
    $stmt->bindParam(':b', $category);
...
$sql = "SELECT * FROM upcoming_products";
$stmt = $con->prepare($sql);
$stmt->execute();
$result_upcoming = $stmt->fetchAll();
...
if (isset($_POST['delete'])) {
    ...
    switch ($_POST['delete_type']) {
        ...
        case 'upcoming_products':
            $stmt = $con->prepare("DELETE FROM upcoming_products WHERE upcoming_id=:deleteId");
            break;
        default:
...
if (isset($_POST['edit'])) {
    ...
    switch ($_POST['delete_type']) {
        case 'users':
            ...
            break;
        case 'products':
            header("Location: ../edit-product.php?product_id=" . $editId);
            break;
        case 'upcoming_products':
            header("Location: ../edit-upcoming.php?upcoming_id=" . $editId);
            break;
        default:
            echo "Invalid delete type";
            exit;
...
<body>
                        ...
                        <ul class="nav navbar-nav menu_nav ml-auto">
                            <li class="nav-item submenu dropdown active">
                                ...
                                <ul class="dropdown-menu">
                                    ...
                                    <li class="nav-item"><a class="nav-link" href="tables.php">Admin Home</a></li>
                            ...
                            <li class="nav-item submenu dropdown">
                                ...
                                <ul class="dropdown-menu">
                                    ...
                                    <li class="nav-item active"><a class="nav-link" href="elements.php">Elements</a>
                                    </li>
                                </ul>
                            </li>
                            ...
                            <?php
								if(isset($_SESSION['id']) || $_SESSION['id'] != 1){
									echo '<li class="nav-item submenu dropdown ">
											...
											<ul class="dropdown-menu">
												<li class="nav-item"><a class="nav-link" href="../edit.php?user_id='. $_SESSION['id'] .'">Edit Profile</a></li>
    ...
    <div class="whole-wrap pb-100">
        <div class="container">
            ...
            <!--UPCOMING PRODUCT TABLES-->
            <div class="section-top-border mb-5">
                <h3 class="mb-30">Upcoming Product Table</h3>
                <div class="progress-table-wrap">
                    <?php
                        if($result_upcoming > 0) {
                            echo '<div class="progress-table">';
                            echo '<div class="table-head">
                            <div class="serial">Id</div>
                            <div class="country">images</div>
                        ...
                        foreach ($result_upcoming as $row){
                            echo '<div class="table-row ">';
                            echo '<div class="serial">'. $row["upcoming_id"] . '</div>';
                            echo '<div class="country"> <img class="img-fluid img-thumbnail" style="width: 100px; height: 100px;" src="../../public/img/upcoming/' . $row['images_upcoming'] .'" alt=""></div>';
                            echo '<div class="visit">'. $row["product_name"] . '</div>';
                            echo '<div class="visit">'. $row["category"] . '</div>';
                            echo '<div class="visit">'. $row["price"] . '</div>';
                            echo '<div class="visit">'. $row["quantity"] . '</div>';
                            echo "<div class='visit'>
                                    <form method='post'>
                                        <div class='btn-group' role='group' >
                                        <button type='submit' name='edit' class='btn btn-primary' value='" . $row["upcoming_id"] . "'><i class='fa fa-pencil-square-o' aria-hidden='true'></i></button>
                                        <input type='hidden' name='delete_type' value='upcoming_products'>
                                        <button type='submit' name='delete' class='btn btn-danger' value='" . $row["upcoming_id"] . "'><i class='fa fa-trash' aria-hidden='true'></i></button>
                                        </div>
                                    </form>
                                </div>";
                            echo '</div>';
            ...
            <div class="section-top-border mb-5">
                <div class="row">
                    ...
                    <div class="col-lg-12 col-md-12 mb-5">
                        <h3 class="mb-30">Create Upcoming Product</h3>
                        <form class="contact_form" action="" method="post" enctype="multipart/form-data">
                            <div class="mt-10">
                                <input type="text" name="product_name" placeholder="Product Name" required class="single-input-primary">
                            </div>
                            ...
                            <div class="mt-10">
                                <input type="file" name="images_upcoming"  required class="single-input-primary">
                            </div>
                            <div class="col-md-12 text-center">
                                <button type="submit" value="submit" name="create-upcoming" class="primary-btn e-large mt-3">Submit Now</button>
                            </div>
                        </form>
                    </div>
                </div>
